...
namespace Framework
{
	namespace MeshUtility
	{
		...
		constexpr auto Interleave( const VertexAttributeArray& vertex_attribute_array_first, const VertexAttributeArrays& ... vertex_attribute_array_pack )
		{
			...
			using CoordinateType = std::remove_reference_t< decltype( vertex_attribute_array_first[ 0 ] ) >::CoordinateType;

			std::array< Math::Vector< CoordinateType, attribute_count >, element_count > interleaved_vertex_attribute_array;
